(function(){
  ...
  const absoluteUrl=value=>new URL(value,location.origin).href;
  ...
  function renderArticle(){
    ...
    const title=`${localized(item,'title')} — Brainwashed`;
    const description=localized(item,'excerpt').replace(/\s+/g,' ').trim();
    const canonical=`${location.origin}/article.html?slug=${encodeURIComponent(item.slug)}`;
    const image=absoluteUrl(imageUrl(item.image));
    document.title=title;
    document.querySelector('meta[name="description"]')?.setAttribute('content',description);
    document.querySelector('#article-canonical')?.setAttribute('href',canonical);
    setMeta('#article-og-title',title);setMeta('#article-og-description',description);setMeta('#article-og-url',canonical);setMeta('#article-og-image',image);
    setMeta('#article-twitter-title',title);setMeta('#article-twitter-description',description);setMeta('#article-twitter-image',image);
